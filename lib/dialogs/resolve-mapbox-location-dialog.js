...
function register(library, apiKey, country = null) {
  ...
  library.dialog('location-resolve-dialog', createLocationResolveDialog(apiKey, country));
}
...
function createLocationResolveDialog(apiKey, country) {
    ...
    .onBegin(function (session, args) {
      if (!args.skipDialogPrompt) {
        ...
        if ('prompt' in args){
          ...
          session.send(promptSuffix).sendBatch();
        }
      }else{
        findLocation(apiKey, country, session)
      }
    }).onDefault(function (session) {
      findLocation(apiKey, country, session)
    });
}
function findLocation(apiKey, country, session){

  let uri = findLocationByQueryUrl + encodeURIComponent(session.message.text) + '.json?access_token=' + apiKey;
  if (country != null) {
    uri += '&country=' + country;
  }
