        ...
        .onBegin(function (session, args) {
            ...
            for (var i = 0; i < entities.length; i++) {
                if (entities[i].type == "Place" && entities[i].geo && entities[i].geo.latitude && entities[i].geo.longitude) {
                  ...
                  if (connection == 'google') {
                    session.replaceDialog('resolve-google-location-dialog', args);
                    return;
                  } else if (connection == 'mapbox') {
                    session.replaceDialog('resolve-mapbox-location-dialog', args);
                    return;
                  } else {
                    session.replaceDialog('resolve-bing-location-dialog', args);
                    return;
                  }
                }
            }
