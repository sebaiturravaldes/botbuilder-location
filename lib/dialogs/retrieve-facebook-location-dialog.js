...
var connection = '';
function register(library, conn, apiKey) {
    connection = conn;
    library.dialog('retrieve-facebook-location-dialog', createDialog(conn, apiKey));
        ...
        function (session, results, next) {
            if (session.dialogData.args.reverseGeocode && results.response && results.response.place) {
                if (connection == 'google') {
                    ...
                    session.endDialogWithResult({response: {place: buildLocationFromGeo(place.geometry.coordinates[1], place.geometry.coordinates[0])}});
    ...
    return common.createBaseDialog()
        .onBegin(function (session, args) {
            ...
            if (entities.length == 0) {
              if (connection == 'google') {
                session.replaceDialog('resolve-google-location-dialog', {skipDialogPrompt: true});
                return;
              } else if (connection == 'mapbox') {
                session.replaceDialog('resolve-mapbox-location-dialog', {skipDialogPrompt: true});
                return;
              } else {
                session.replaceDialog('resolve-bing-location-dialog', {skipDialogPrompt: true});
                return;
              }
            } else{
